...
/**
 * \ingroup scheduler组内调度程序
 * The specific event scheduler implementation to use.要使用的特定事件调度程序实现
 *
...
TimePrinter (std::ostream &os)
{
  ...
  if (Time::GetResolution () == Time::NS)
    ...
    {
      os << std::fixed << std::setprecision (15) << Simulator::Now ().As (Time::S);//As返回time和unit
    }
...
/**
 * \ingroup logging
 * Default node id printer implementation.默认的节点ID打印机实现。
 * 
...
/**
 * \ingroup simulator
 * \brief Get the static SimulatorImpl instance.获取静态SimulatorImpl实例
 * \return The SimulatorImpl instance pointer.返回SimulatorImpl实例指针
 */
...
/**
 * \ingroup simulator
 * \brief Get the SimulatorImpl singleton.简要获取SimulatorImpl单例。
 * \return The singleton pointer.返回单例指针
    ...
    {
      {
        ...
        ////SimulatorImpl返回对象id，Create<SimulatorImpl> ()返回一个有n1,n2,...n8的集合
