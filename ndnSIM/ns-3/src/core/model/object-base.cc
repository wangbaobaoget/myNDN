...
ObjectBase::ConstructSelf (const AttributeConstructionList &attributes)
{
  ...
  TypeId tid = GetInstanceTypeId ();//newtype对象并初始化
  do {
      ...
      for (uint32_t i = 0; i < tid.GetAttributeN (); i++)
        {
...
#ifdef HAVE_GETENV
          // No matching attribute value so we try to look at the env var.没有匹配的属性值，因此我们尝试查看环境变量
          ...
          // No matching attribute value so we try to set the default value.没有匹配的属性值，因此我们尝试设置默认值。
