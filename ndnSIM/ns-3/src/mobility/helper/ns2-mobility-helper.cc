...
struct ParseResult
{
  ...
  std::vector<bool> has_ival; //!< points if a tokens has an int value指出令牌是否具有int值
...
{
  Vector m_startPosition;     //!< Start position of last movement最后移动的开始位置
  ...
  EventId m_stopEvent;        //!< Event scheduling node's stop. May be canceled if needed.事件调度节点的停止。如果需要可以取消。
...
static void setnodestatus(int unodeid, double x,double y,double speed);


/**
 * Set waypoints and speed for movement.
 */
static DestinationPoint SetMovement (Ptr<ConstantVelocityMobilityModel> model, Vector lastPos, int inodeid,double at,
...
Ns2MobilityHelper::Ns2MobilityHelper (std::string filename)
  : m_filename (filename)//c++中初始化列表，冒号后面的是初始化列表，对类的成员赋值的
{
...
static void setnodestatus(int unodeid,double x,double y,double speed)
{
  ...
  nodestatus[unodeid][0]=x;
  nodestatus[unodeid][1]=y;
  nodestatus[unodeid][2]=speed;
...
Ns2MobilityHelper::ConfigNodesMovements (const ObjectStore &store) const
{
  ...
  //在整个文件中查找初始节点//的位置，以使此帮助程序健壮地处理//初始节点位于末尾的跟踪文件。
  std::ifstream file (m_filename.c_str (), std::ios::in);//ofstream是从内存到硬盘，ifstream是从硬盘到内存，其实所谓的流缓冲就是内存空间
  if (file.is_open ())
    {
      while (!file.eof () )//成员函数eof()用来检测是否到达文件尾，如果到达文件尾返回非0值，否则返回0。原型是int eof();
        {
          int         iNodeId = 0;
          std::string nodeId;
          std::string line;
          ...
          ParseResult pr = ParseNs2Line (line); // Parse line and obtain tokens解析行并获取令牌，返回ret（包括一些和节点id相关的解析集合）

          // Check if the line corresponds with setting the initial
          // node positions检查该行是否与设置初始//节点位置相对应
          if (pr.tokens.size () != 4)
            {
              continue;
            }
          ...
          // get mobility model of node获取节点的移动性模型
          ...
          /*
           * In this case a initial position is being seted在这种情况下，将设置初始位置
           * line like $node_(0) set X_ 151.05190721688197
           */
          if (IsSetInitialPos (pr))
            {
              DestinationPoint point;//包含了节点的一些速度位置信息
              //                                                    coord         coord value
              point.m_finalPosition = SetInitialPosition (model, pr.tokens[2], pr.dvals[3]);
              last_pos[iNodeId] = point;
              nodestatus[iNodeId][0]=point.m_finalPosition.x;
              nodestatus[iNodeId][1]=point.m_finalPosition.y;
              nodestatus[iNodeId][2]=0;
  ...
  //*****************************************************************
  // Parse the file a second time to get the rest of its values第二次解析文件以获取其余值
  //*****************************************************************
          ...
          else // NOW EVENTS TO BE SCHEDULED
            {
              ...
              if (IsSchedMobilityPos (pr))
                {
                  if (last_pos[iNodeId].m_targetArrivalTime > at)
                    {
                      ...
                      std::cout<<"final  point ="<< last_pos[iNodeId].m_finalPosition <<",actuclly reached = "<<reached;
                  ...
                  std::cout<<"last_pos[iNodeId].m_finalPosition,"<<last_pos[iNodeId].m_finalPosition<<std::endl;
                  std::cout<<"at"<<at<<std::endl;
                  std::cout<<"X"<<pr.dvals[5]<<std::endl;
                  std::cout<<"Y"<<pr.dvals[6]<<std::endl;
                  std::cout<<"velociyt"<<pr.dvals[7]<<std::endl;
                  last_pos[iNodeId] = SetMovement (model, last_pos[iNodeId].m_finalPosition,iNodeId, at, pr.dvals[5], pr.dvals[6], pr.dvals[7]);

                  int w = (int)pr.dvals[5];
                  std::cout<<"change test int="<<w<<"old="<<pr.dvals[5]<<std::endl; 
                 std::cout<<"type of pr.dvals[5]"<<typeid(pr.dvals[5]).name()<<std::endl;
 


                 // nodestatus[iNodeId][0]=pr.dvals[5];
                 // nodestatus[iNodeId][1]=pr.dvals[6];
                 // nodestatus[iNodeId][2]=pr.dvals[7];
                // std::cout<<"nodestatus4:x"<<nodestatus[4][0]<<"y"<<nodestatus[4][1]<<"velocity"<<nodestatus[4][3]<<std::endl;
                // std::cout<<"nodestatus3:x"<<nodestatus[3][0]<<"y"<<nodestatus[3][1]<<"velocity"<<nodestatus[3][3]<<std::endl;
                // std::cout<<"nodestatus2:x"<<nodestatus[2][0]<<"y"<<nodestatus[2][1]<<"velocity"<<nodestatus[2][3]<<std::endl;
                // std::cout<<"nodestatus1:x"<<nodestatus[1][0]<<"y"<<nodestatus[2][1]<<"velocity"<<nodestatus[1][3]<<std::endl;
                // std::cout<<"nodestatus0:x"<<nodestatus[0][0]<<"y"<<nodestatus[0][1]<<"velocity"<<nodestatus[0][3]<<std::endl;
                // std::cout<<"*************************************"<<std::endl;
                  // Log new position
                  NS_LOG_DEBUG ("Positions after parse for node " << iNodeId << " " << nodeId << " position =" << last_pos[iNodeId].m_finalPosition);

                  //add simulator::schedual(at,void set nodestatus,x,y,z)
                  // Simulator::Schedule (Seconds (at), &setnodestatus, iNodeId,pr.dvals[5],pr.dvals[6],pr.dvals[7]);
                }
               ...
               */
              else if (IsSchedSetPos (pr))
                {
                  ...
                  std::cout<<"-----------------------------------------------------------------------------------------------------------------------------------------------------------------------IsSchedSetPos--------------------------------------------------------------------------"<<std::endl;
...
ParseNs2Line (const std::string& str)
{
  ...
  // If line hasn't a correct node Id如果行没有正确的节点ID
  ...
  // if it is a scheduled set _[XYZ] or a setdest I need to remove the last "如果是预定集_ [XYZ]或setdest，我需要删除最后一个
  // and re-calculate values并重新计算值
...
std::string
TrimNs2Line (const std::string& s)//处理文件，将不相关的东西删除
{
  std::string ret = s;
  ...
  while (ret.size () > 0 && (isblank (ret[ret.size () - 1]) || (ret[ret.size () - 1] == ';')))
    {
      ret.erase (ret.size () - 1, 1); // Removes blank spaces from at end of line从行尾删除空格
    }
...
DestinationPoint
SetMovement (Ptr<ConstantVelocityMobilityModel> model, Vector last_pos,int inodeid,double at,
             double xFinalPosition, double yFinalPosition, double speed)
{
  ...
  if (speed > 0)
    {
      ...
      // Set the Values
      std::cout<<"inodeid "<<inodeid<<std::endl;
      std::cout<<"x"<<xFinalPosition<<std::endl;
      

      Simulator::Schedule (Seconds (at), &ConstantVelocityMobilityModel::SetVelocity, model, Vector (xSpeed, ySpeed, zSpeed));
      Simulator::Schedule (Seconds (at), &setnodestatus, inodeid,xFinalPosition,yFinalPosition,speed);
      std::cout<<"nodestatus4:x"<<nodestatus[4][0]<<"y"<<nodestatus[4][1]<<"velocity"<<nodestatus[4][3]<<std::endl;
                std::cout<<"nodestatus3:x"<<nodestatus[3][0]<<"y"<<nodestatus[3][1]<<"velocity"<<nodestatus[3][3]<<std::endl;
                std::cout<<"nodestatus2:x"<<nodestatus[2][0]<<"y"<<nodestatus[2][1]<<"velocity"<<nodestatus[2][3]<<std::endl;
                std::cout<<"nodestatus1:x"<<nodestatus[1][0]<<"y"<<nodestatus[2][1]<<"velocity"<<nodestatus[1][3]<<std::endl;
