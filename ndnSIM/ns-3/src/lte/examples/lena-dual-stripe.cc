...
FemtocellBlockAllocator::Create ()
{
  ...
  //创建GridBuildingAllocator类的对象 
  //GridBuildingAllocator类 用来在矩形2d网格上分配建筑物
  Ptr<GridBuildingAllocator>  gridBuildingAllocator;
  gridBuildingAllocator = CreateObject<GridBuildingAllocator> ();
  gridBuildingAllocator->SetAttribute ("GridWidth", UintegerValue (1));//设置要创建的建筑物的属性
  ...
  gridBuildingAllocator->Create (2);//在一组网格上分配建筑物，n为要创建的建筑物个数，返回一个包含新创建的建筑物的容器
}
